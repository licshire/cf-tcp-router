...
`
			...
			Context("when Configure is called multiple times with the same routing table", func() {
				...
				It("does not call script more than once", func() {
					Consistently(func() int {
						err = haproxyConfigurer.Configure(routingTable)
						Expect(err).ShouldNot(HaveOccurred())
						return scriptRunner.RunCallCount()
					}).Should(Equal(1))
				})
			})
		})
